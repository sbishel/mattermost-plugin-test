...
## When BUILD_FOR_TEST=1 (set automatically by the test targets), only linux/$(DEFAULT_GOARCH) is built
## since the integration test container always runs Linux.
.PHONY: server
server:
ifneq ($(HAS_SERVER),)
ifneq ($(MM_DEBUG),)
...
ifneq ($(MM_SERVICESETTINGS_ENABLEDEVELOPER),)
	...
	@if [ "$(BUILD_FOR_TEST)" = "1" ]; then \
		...
		cd server && env CGO_ENABLED=0 GOOS=linux GOARCH=$(DEFAULT_GOARCH) $(GO) build $(GO_BUILD_FLAGS) $(GO_BUILD_GCFLAGS) -trimpath -o dist/plugin-linux-$(DEFAULT_GOARCH); \
	else \
		cd server && \
		env CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(GO) build $(GO_BUILD_FLAGS) $(GO_BUILD_GCFLAGS) -trimpath -o dist/plugin-linux-amd64 && \
		env CGO_ENABLED=0 GOOS=linux GOARCH=arm64 $(GO) build $(GO_BUILD_FLAGS) $(GO_BUILD_GCFLAGS) -trimpath -o dist/plugin-linux-arm64 && \
		env CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 $(GO) build $(GO_BUILD_FLAGS) $(GO_BUILD_GCFLAGS) -trimpath -o dist/plugin-darwin-amd64 && \
		env CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 $(GO) build $(GO_BUILD_FLAGS) $(GO_BUILD_GCFLAGS) -trimpath -o dist/plugin-darwin-arm64 && \
		env CGO_ENABLED=0 GOOS=windows GOARCH=amd64 $(GO) build $(GO_BUILD_FLAGS) $(GO_BUILD_GCFLAGS) -trimpath -o dist/plugin-windows-amd64.exe; \
	fi
endif
endif
...
## Depends on dist so the plugin bundle is available for integration tests.
## BUILD_FOR_TEST=1 restricts the server build to linux/$(DEFAULT_GOARCH) only.
.PHONY: test
...
.PHONY: test-ci
test-ci: BUILD_FOR_TEST = 1
test-ci: dist install-go-tools
ifneq ($(HAS_SERVER),)
